...
BEGIN_MESSAGE_MAP(CMFCApplication3App, CWinApp)
	...
	ON_COMMAND(ID_MNU_Test2, &CMFCApplication3App::OnMnuTest22)
...
void CMFCApplication3App::OnMnuTest1()
{
	...
	CDlgTest dlg;
	if (dlg.DoModal() == IDOK) AfxMessageBox(L"ㅇㅋ");
	else AfxMessageBox(L"뿡");
	//dlg.DoModal();
}


void CMFCApplication3App::OnMnuTest22()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
...
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CDrawTest* dlg = new CDrawTest();	//반드시 초기화, CDrawTest* dlg = new CDrawTest;해도 오류 안뜸?
	dlg->DoModal();		//다이얼로그클래스 메뉴와 연결
}
